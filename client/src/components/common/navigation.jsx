...
const Navigation = () => {
  ...
  const { auth: {isAuthenticated, userInfo }} = useSelector(state => state)
  const { onNavLinkClickHandle, getNavItemClassName } = useNavigationUtils()

  useEffect(() => {
    isTrueState(isAuthenticated) ? setNavigationLinks(pagePaths.privateRoutes) : setNavigationLinks(pagePaths.publicRoutes)
  }, [isAuthenticated])
  ...
  return (
    <header className='container-md'>
      <nav className="navbar navbar-expand-lg mt-5">
        ...
        <div className={`container-md d-flex justify-content-${isTrueState(isAuthenticated) ? 'start' : 'end'} nav-links`}>
          {navigationLinks.filter(page => !page.ignoreNav).map((page, idx) => (
            <span key={idx} onClick={() => onNavLinkClickHandle(page.path)}
                  className={getNavItemClassName(page.path)}>{page.title.toUpperCase()}</span>
          ))}
        </div>
        {isTrueState(isAuthenticated) ? <div className='container-md ml-5 d-flex justify-content-end nav-links'>
