...
const ThirdPartyLogin = () => {
  ...
  const onGoogleLogin = useGoogleLogin({
    onSuccess: ({ code }) => dispatch(thirdPartyLogin(code, ThirdPartyLoginEnum.GOOGLE)),
  ...
  const onFacebookLogin = () => {
    window.FB.login(({ authResponse: {accessToken}, status }) => {
      if(status === 'connected') {
  ...
  return (
    <div className='d-flex flex-row justify-content-around mt-4'>
      ...
      <img onClick={() => onFacebookLogin()} className='custom-href' src={FacebookLogo} style={{ mixBlendMode: 'color-dodge' }} alt='facebook-logo' />

      <LoginGithub scope='user,user:email' className='github-button' buttonText='' children={<img className='custom-href' src={GithubLogo} alt='github-logo' /> } clientId={githubClientId} onSuccess={(response) => onGithubLogin(response)} />
    </div>
  )
}
