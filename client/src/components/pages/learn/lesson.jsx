      ...
      <div className='d-flex flex-row justify-content-between'>
        <div className='difficulty-container d-flex flex-row'>
          ...
          {generateRangeArray(difficulty).map((val) => (
            <img key={val} alt='star' className='single-star' src={starFillVector} />
          ))}
          {generateRangeArray(5 - difficulty).map((val) => (
            <img key={val} alt='star' className='single-star' src={starNonFillVector} />
          ))}
        </div>
        <button disabled={getIsDisabled()} className={`justify-content-end ${getIsDisabled() ? 'disabled' : ''}`} style={{ color: '#7000B1' }}>{isDone ? 'READ AGAIN' : 'LEARN MORE'}</button>
      </div>
    </div>
  )
}
